...
function Upload-GitHubMedia([string]$Path, [string]$Prefix) {
    ...
    if (!$githubToken) { return Upload-CatboxMedia $Path }
...
function Upload-CatboxMedia([string]$Path) {
    $response = & curl.exe --ssl-no-revoke --silent --show-error --connect-timeout 10 --max-time 60 `
        -F "reqtype=fileupload" `
        -F "fileToUpload=@$Path" `
        "https://catbox.moe/user/api.php"
    if ($LASTEXITCODE -ne 0 -or $response -notmatch "^https://") {
        throw "Could not create a public media URL. Add GITHUB_TOKEN or try again later. Response: $response"
    }
